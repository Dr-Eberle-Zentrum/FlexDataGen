...
// ============================================================================
    ...
    // ------------------------------------------------------------------------
        ...
        foreach($this->tables as $table_name => $table) {
            ...
            if(is_array($table['rows'])) { // table rows are constructed by combinations of columns
                $this->uniqueCombinations = isset($table['rows'][0]['uniqueCombinations'])
                    ? $table['rows'][0]['uniqueCombinations']
                    : true;
                $this->rowCounts[$table_name] = $this->generateCombinations(
                    $table['rows']
                );
            }
    ...
    // ------------------------------------------------------------------------
        for($i = count($this->combinedValues) - 1; $i >= 0; $i--) {
            $existingCombination = $this->combinedValues[$i];
            // if first column doesnt match were fine, since we start at the end
            // and the first column is "grouped"
            if($existingCombination[0] !== $combination[0])
                return false;
            $duplicate = true;
            foreach($existingCombination as $index => $value) {
    ...
    // ------------------------------------------------------------------------
        if($columnIndex >= count($this->combinedColumnInfos)) {
            if($this->isDuplicate($prevCombinations))
                return false;

            // now we have a full valid record in $prevCombinations
            for($i = 0; $i < count($prevCombinations); $i++) {
                $this->combinedValues[] = $prevCombinations;
                $this->cur
                    [$this->curTableName]
        ...
        if($columnIndex === 0) { // special treatment: pick by probability
            ...
            foreach($pool as $value) {
                ...
                do {
                    ...
                    $valid = $this->addToCombination($columnIndex + 1, $prevCombinations);
                }
                while($this->uniqueCombinations && !$valid);
            }
        }
        else {
            ...
            for($i = 0; $i < $numRecords; $i++) {
                ...
                if(!$this->addToCombination($columnIndex + 1, $prevCombinations)) {
                    $values[$i] = $this->columnGenerators[$columnIndex]->getData(1)[0]; // generate new value
                    $i--; // try again
                    continue;
                }
            }
    ...
    // ------------------------------------------------------------------------
        ...
        foreach($columns as $index => $column) {
            ...
            if($index > 0 && $this->columnGenerators[$index]->isUnique()) {
                throw new Exception('Single column cannot be set to be unique in combinations generator! ' .
                    'At this stage only unique *combinations* are generated!');
            }
            $this->occurrenceDistributions[$index] = (
                ...
                : Helper::getInstanceArgs($column['combinations']['distribution'], [
                    ...
                    'options' => isset($column['combinations']['options']) 
                        ? $column['combinations']['options']
                        : []
                ])
            );
        }
    ...
    // ------------------------------------------------------------------------
    public function setColumnValues(
        $table_name, 
        $column_name,
        $values
    ) {
    // ------------------------------------------------------------------------
        if(!isset($this->cur[$table_name]))
            $this->cur[$table_name] = [];
        $this->cur[$table_name][$column_name] = $values;
        return $this;
    }
